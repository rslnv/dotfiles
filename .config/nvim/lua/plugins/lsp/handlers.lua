...
local function lsp_keymaps(bufnr)
	...
	local status_which_key_ok, which_key = pcall(require, "which-key")
	if not status_which_key_ok then
		print("Unable to set which-key lsp hotkeys")
		return
	end
	...
	which_key.register({
		l = {
			...
			j = { "<cmd>lua vim.diagnostic.goto_next({buffer=0})<CR>", "Next diagnostic", buffer = bufnr },
			k = { "<cmd>lua vim.diagnostic.goto_prev({buffer=0})<CR>", "Prev diagnostic", buffer = bufnr },
			q = { "<cmd>lua vim.diagnostic.setloclist()<CR>", "Diagnostics to location list", buffer = bufnr },
		},
	}, { prefix = "<leader>" })
end
